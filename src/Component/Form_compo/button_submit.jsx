...
const Registe_Dog = () => {
  ...
  const { move_plusdog } = useNavigationHandlers();
  const [selectedDog, setSelectedDog] = useState('');
  const [isOpen, setIsOpen] = useState(false); // 강아지 정렬 창 여닫이
    ...
    <div className="header_profile">
      {user ? (
        <>
          ...
          {user.userType === 'regular' ? <Registe_Dog /> : <div className="header_profile_username">수의사님</div>}
          <Button_Logout />
        </>
      ) : (
        <Button_Login />
